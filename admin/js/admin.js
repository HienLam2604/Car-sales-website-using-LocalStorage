...
function showThongtinSanPham(index){
	...
	for(var i = start; i<end;i++){
		...
		if(productArray[i].loai=='xeso')
			loai = loai + '<div><h1>'+'Xe Số'+'</h1></div>';
		else if(productArray[i].loai=='tayga')
			loai = loai + '<div><h1>'+'Xe Tay ga'+'</h1></div>';
		else if(productArray[i].loai=='taycon')
			loai = loai + '<div><h1>'+'Xe Côn tay'+'</h1></div>';
		else
			loai = loai + '<div><h1>'+'Xe Môtô'+'</h1></div>';
...
function addProduct(x){
	...
	var loai = document.getElementById("loai1").value;
	...
	var product={productId:productArray[productArray.length -1].productId + 1, brand:thuonghieu,
		loai:loai,img:fileAnh[2], name:ten, price:gia};
...
function repairProduct(id){
	...
	for(var i=0;i<productArray.length;i++){
		if(productArray[i].productId==id) {
			...
			document.getElementById("loai").value = productArray[i].loai;
