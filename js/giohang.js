...
function Order(){
		...
		if(soluong > 99999999999){
			alert("Hiện tại kho không đủ số lượng hàng. Nhấn ok để xóa giỏ hàng tự động!");
				localStorage.removeItem('cart');
			showCartOrder();
			return false;
		}	
...
function createNewUser(x){
	...
	if(!fullname ){
		...
		if(fullname.length<10){
			document.getElementById('errorhoten').style.display = 'block';
			document.getElementById('errorhoten').innerHTML = 'Tên tài khoản phải trên 10 ký tự và viết liền!';
			flag = false;
		}else {
	...
	if(!email){
		...
		if(email.length>19){
			document.getElementById('erroremail').style.display = 'block';
			document.getElementById('erroremail').innerHTML = 'Email phải dưới 19 ký tự và viết liền!';
			flag = false;
		}else {
	...
	if(ktrakhoangtrang(pass)==true){
		document.getElementById('errormk').style.display = 'block';
		document.getElementById('errormk').innerHTML = 'Mật khẩu phải trên 8 ký tự và viết liền!';
		flag = false;
	}
	if(ktrakhoangtrang(repass)==true){
		document.getElementById('errormk1').style.display = 'block';
		document.getElementById('errormk1').innerHTML = 'Mật khẩu phải trên 8 ký tự và viết liền!';
		flag = false;
	}
	if(flag==false){
		return false;
